...
export async function PUT(
  ...
  try {
    ...
    const updatedUserMain = await User.findByIdAndUpdate(
      params.id,
      updateData,
    ...
    try {
      ...
      // Find user in tenant DB by email (since _id might be different)
      ...
      console.error('Error updating user in tenant database:', tenantError);
...
export async function DELETE(
  ...
  try {
    ...
    try {
      ...
      console.error('Error deleting user from tenant database:', tenantError);
      // Don't fail the request, just log the error
    }
