...
export async function POST(request: NextRequest) {
  try {
    ...
    const newUserMain = await User.create({
      name,
    ...
    try {
      ...
      // Rollback: Delete from main database if tenant creation fails
      await User.findByIdAndDelete(newUserMain._id);
      
      return NextResponse.json(
        { error: 'Failed to create user in organization database' },
        { status: 500 }
      );
    }
    ...
    const userResponse = {
      _id: newUserMain._id,
      name: newUserMain.name,
      email: newUserMain.email,
      role: newUserMain.role,
      organizationId: newUserMain.organizationId,
      isActive: newUserMain.isActive,
      createdAt: newUserMain.createdAt,
    };
