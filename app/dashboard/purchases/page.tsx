...
//     const bottlesPerCaret = product.bottlesPerCaret || 12;

//     const totalBottles = (data.carets * bottlesPerCaret) + data.pieces;
//     const amount = data.carets * data.pricePerCaret + data.pieces * (data.pricePerCaret / bottlesPerCaret);
...
//                           min="0"
//                           value={watchCarets}
//                           onChange={(e) => setValue("carets", Number(e.target.value))}
//                         />
//                         {errors.carets && (
...
//                           type="number"
//                           min="0"
//                           max={products.find((purchase) => purchase._id === watchProductId)?.bottlesPerCaret}
//                           value={watchPieces}
//                           onChange={(e) => setValue("pieces", Number(e.target.value))}
//                         />
//                         {errors.pieces && (
...
//                           {(() => {
//                             const selectedProduct = products.find(p => p._id === watchProductId);
//                             const bottlesPerCaret = selectedProduct?.bottlesPerCaret || 12;
//                             const amount = (watchCarets * watchPrice) + (watchPieces * (watchPrice / bottlesPerCaret));
...
const Index = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch vendors
      const vendorsRes = await apiFetch('/api/vendors');
      ...
      // Fetch purchases
      const purchasesRes = await apiFetch('/api/purchases');
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="flex flex-1 flex-col lg:flex-row overflow-hidden">
        ...
        <main className="lg:basis-[55%] overflow-y-auto border-b lg:border-b-0 lg:border-r">
          <div className="container mx-auto px-6 py-6 max-w-5xl">
            <div className="space-y-6">
              <Card>
                <CardHeader>
        ...
        {/* Purchase Cart - 45% on large, full width on small */}
        <aside className="lg:basis-[45%] bg-card flex-shrink-0 overflow-y-auto border-t lg:border-t-0 lg:border-l">
          <div className="p-6 space-y-6">
            <div className="space-y-2">
              <h2 className="text-xl font-bold flex items-center gap-2">
                <ShoppingCart className="h-5 w-5 text-primary" />
            ...
            {currentItems.length > 0 ? (
              <>
                ...
                <Button onClick={onSavePurchase} className="w-full" disabled={loading}>
                  {loading ? (
                    <>
